...
class PrefLiveDelegate<T>(private val defaultValue: T, val fieldKey:String? = null) {
    ...
    operator fun provideDelegate(
        ...
        return object : ReadOnlyProperty<PrefManager, LiveData<T>> {
            override fun getValue(thisRef: PrefManager, property: KProperty<*>): LiveData<T> {
                if(!::storedValue.isInitialized) storedValue = SharedPreferenceLiveData(thisRef.preferences, key,defaultValue)
                return storedValue
            }
        }
    }
